...
const getTemplate = (value, fallbackStore = {}) => {
  ...
  return templateMap[normalized] || templateMap[inferTemplateKey(fallbackStore)] || storeTemplates[0];
};
...
const createDefaultMessaging = (template, storeName) => {
  return {
    tagline: `${template.typeLabel} storefront for ${storeName}.`,
...
const getStoreTheme = (store = {}) => {
  const template = getTemplate(store.template_key || store.store_type, store);
  const fontPreset = getFontPreset(store.font_preset, template.fontPreset);

  return {
    ...template,
    templateKey: template.key,
    storeType: String(store.store_type || template.key),
...
const getStoreConfiguration = (payload = {}, fallbackStore = {}) => {
  const template = getTemplate(payload.template_key || payload.store_type, fallbackStore);
  const fontPreset = getFontPreset(payload.font_preset, template.fontPreset);
  const storeName = String(payload.name || fallbackStore.name || 'New Store').trim() || 'New Store';
  const defaults = createDefaultMessaging(template, storeName);

  return {
    store_type: String(payload.store_type || template.key).trim() || template.key,
